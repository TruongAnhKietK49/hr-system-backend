...
class EmployeeRepository {
  async findAllByRoleScope(scope) {
    ...
    addInput(request, 'RequesterEmployeeID', SqlTypes.VarChar, scope.employeeId);
    const result = await request.execute(resolveEmployeeListProcedure(scope.role));
  ...
  async findByIdScoped(scope) {
    ...
    const result = await request.execute(resolveEmployeeDetailProcedure(scope.role));
  ...
  async updateByRole(payload) {
    ...
    if (payload.role === 'HR Manager') {
      ...
      addInput(request, 'EmploymentStatus', SqlTypes.NVarChar, payload.employmentStatus);
      addInput(request, 'IsActive', SqlTypes.Bit, payload.isActive);
    }

    const result = await request.execute(resolveEmployeeUpdateProcedure(payload.role));
    return result.recordset[0] || null;
  }
}
