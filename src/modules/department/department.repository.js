...
class DepartmentRepository {
  ...
  async searchManagerCandidates({ keyword, limit }) {
    const pool = await getPool();
    const request = pool.request();

    addInput(request, "Keyword", SqlTypes.NVarChar, keyword || null);
    addInput(request, "Limit", SqlTypes.Int, limit);

    const result = await request.execute(
      "sp_Department_SearchManagerCandidates",
    );
