...
class RequestService {
  async create(user, payload) {
    if (![ROLES.HR_STAFF, ROLES.HR_MANAGER].includes(user.role)) {
      throw new AppError(
        "Only HR Staff or HR Manager can create HR request",
        403,
      );
    }
  ...
  async getAll(user) {
    if (
      ![ROLES.HR_STAFF, ROLES.HR_MANAGER, ROLES.DIRECTOR].includes(user.role)
    ) {
  ...
  async getById(user, requestId) {
    const request = await requestRepository.findByIdByScope(
      user.employeeId,
      requestId,
    );
    if (!request) {
